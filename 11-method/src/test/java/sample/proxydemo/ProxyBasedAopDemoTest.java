...
public class ProxyBasedAopDemoTest {
	...
	public void interfaceBaseProxy() {
		...
		SimpleMessageService securedProxy = new SimpleMessageService() {
			public String getMessage(int id) {
				if (id != 1) {
					throw new AccessDeniedException("We only allow access to messages with id of 1");
				}
		...
		// notice if we try and cast the proxy to MessageServiceBean we would
		...
		// notice that printMessage is not secured even though
		// MessageServiceBean calls
	...
	public void classBaseProxy() {
		...
		MessageServiceBean securedStub = (MessageServiceBean) securedProxy;
		// notice that the method getFromUser(int id) can be called since we can
		// cast to MessageServiceBean
